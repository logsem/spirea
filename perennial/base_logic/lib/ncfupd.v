...
Definition ncfupd_def `{!invGS Σ, !crashGS Σ} `{Ω : gGenCmras Σ} (E1 E2 : coPset) (P: iProp Σ) : iProp Σ :=
  ∀ q, NC q -∗ |={E1, E2}=> P ∗ NC q.
Definition ncfupd_aux `{!invGS Σ, !crashGS Σ} `{Ω : gGenCmras Σ} : seal (ncfupd_def). Proof. by eexists. Qed.
Definition ncfupd `{!invGS Σ, !crashGS Σ} `{Ω : gGenCmras Σ} := ncfupd_aux.(unseal).
Definition ncfupd_eq `{!invGS Σ, !crashGS Σ} `{Ω : gGenCmras Σ} : ncfupd = ncfupd_def := ncfupd_aux.(seal_eq).

Arguments ncfupd {Σ I C Ω} : rename.
...
Section ncfupd.
Context `{!invGS Σ, Ω : gGenCmras Σ, !crashGS Σ}.
...
Lemma nc_fupd_soundness `{!invGpreS Σ, !crashGpreS Σ} `{!lcGpreS Σ} `{Ω : gGenCmras Σ} n E1 E2 (φ : Prop) :
...
Lemma ncfupd_soundness `{!invGpreS Σ, !crashGpreS Σ} `{!lcGpreS Σ} `{Ω : gGenCmras Σ} n E1 E2 (φ : Prop) :
