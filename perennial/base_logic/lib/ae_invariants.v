...
Definition ae_inv_def `{!invGS Σ} `{Ω : gGenCmras Σ} (k : nat) (mj: option nat) (P : iProp Σ) : iProp Σ :=
  □ ∀ Q E, (match mj with
            | None =>
              match k with
              | O => ▷ P ==∗ ◇ (▷ P ∗ Q)
...
Definition ae_inv {Σ Ω i} := ae_inv_aux.(unseal) Σ Ω i.
Definition ae_inv_eq : @ae_inv = @ae_inv_def := ae_inv_aux.(seal_eq).
#[global]
Instance: Params (@ae_inv) 4 := {}.
...
Section ae_inv.
  ...
  Lemma own_ae_inv_alloc k mj E P : ▷ P -∗ ae_inv_condition P ={E}=∗ own_ae_inv k mj P.
  Proof. iIntros "HP #Hcond". iApply (fupd_split_level_fupd). by iApply (own_ae_inv_alloc_k with "HP Hcond"). Qed.
  ...
  Lemma ae_inv_alloc' k mj E P : ▷ P -∗ ae_inv_condition P -∗ |k,mj={E}=> ae_inv k mj P.
  Proof.
    iIntros "HP #Hcond". iApply own_ae_inv_to_ae_inv.
    by iApply (own_ae_inv_alloc_k E k with "HP").
  Qed.

  Lemma ae_inv_alloc k mj E P : ▷ P -∗ ae_inv_condition P ={E}=∗ ae_inv k mj P.
  Proof.
    iIntros "HP Hcond". iApply own_ae_inv_to_ae_inv.
    by iApply (own_ae_inv_alloc k mj E with "HP").
  Qed.
