...
Section inv.
  ...
  Proof.
    ...
    iDestruct "HI" as (? Ps_mut) "(#Hcond&Hinterp&Hmut)".
  ...
  Lemma own_inv_alloc N E P : ▷ P -∗ inv_condition P ={E}=∗ own_inv N P.
  Proof. iIntros "HP Hcond". iApply (fupd_level_fupd). by iApply (own_inv_alloc0 with "[$]"). Qed.

  (* This does not imply [own_inv_alloc] due to the extra assumption [↑N ⊆ E]. *)
  Lemma own_inv_alloc_open0 N E P :
    ↑N ⊆ E → ⊢ |0={E, E∖↑N}=> own_inv N P ∗ (▷P -∗ inv_condition P -∗ |0={E∖↑N, E}=> True).
  Proof.
    rewrite uPred_fupd_level_eq /uPred_fupd_level_def.
    rewrite uPred_fupd_split_level_eq /uPred_fupd_split_level_def.
    iIntros (Sub) "[Hw HE]".
  ...
  Lemma own_inv_alloc_open N E P :
    ↑N ⊆ E → ⊢ |={E, E∖↑N}=> own_inv N P ∗ (▷P -∗ inv_condition P ={E∖↑N, E}=∗ True).
  Proof.
    iIntros (?). iApply (fupd_level_fupd E _ _ O).
    iMod (own_inv_alloc_open0 N) as "($&Hcl)"; first auto.
    iModIntro. iIntros "H #Hcond". iSpecialize ("Hcl" with "H Hcond"). by iApply fupd_level_fupd.
  Qed.
  ...
  Lemma inv_alloc' k N E P : ▷ P -∗ inv_condition P -∗ |k={E}=> inv N P.
  Proof.
    iIntros "HP #Hcond". iApply own_inv_to_inv.
    iPoseProof (own_inv_alloc0 N E with "HP Hcond") as "H".
    iApply (fupd_level_le with "H"). lia.
  Qed.

  Lemma inv_alloc N E P : ▷ P -∗ inv_condition P ={E}=∗ inv N P.
  Proof.
    iIntros "HP #Hcond". iApply own_inv_to_inv.
    by iApply (own_inv_alloc N E with "HP").
  Qed.

  Lemma inv_alloc_open0 N E P :
    ↑N ⊆ E → ⊢ |0={E, E∖↑N}=> inv N P ∗ (▷P -∗ inv_condition P -∗ |0={E∖↑N, E}=> True).
  Proof.
  ...
  Lemma inv_alloc_open N E P :
    ↑N ⊆ E → ⊢ |={E, E∖↑N}=> inv N P ∗ (▷P -∗ inv_condition P ={E∖↑N, E}=∗ True).
  Proof.
