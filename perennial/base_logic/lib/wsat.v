...
| bi_sch_plainly : bi_schema -> bi_schema
...
Module invGS.
  Definition invR Σ := (authR (gmapUR positive
  ...
  (*   invGS_gen :> genInDepsG Σ Ω (invR Σ) [#]; *)
  (*   invGS_en_gen :> genInDepsG Σ Ω coPset_disjR [#]; *)
  (*   invGS_dis_gen :> genInDepsG Σ Ω (gset_disjR positive) [#]; *)
  (*   mlist_gen :> fmlistGenG (invariant_level_names) Σ Ω; *)
  (* }. *)
...
Fixpoint bi_schema_pre `{!invGS Σ} n (Ps Ps_mut: list (iProp Σ)) wsat (sch: bi_schema) :=
  ...
  | bi_sch_plainly sch => ■ bi_schema_pre n Ps Ps_mut wsat sch
...
Fixpoint bi_schema_interp `{!invGS Σ} {Ω : gGenCmras Σ} n (Ps Ps_mut: list (iProp Σ)) sch {struct n} :=
  match n with
  | O => bi_schema_pre O Ps Ps_mut True%I sch
  | S n' => bi_schema_pre (S n') Ps Ps_mut (wsat_pre n' (bi_schema_interp n') ∗ wsat n')%I sch
  end
  with
  wsat `{!invGS Σ} {Ω : gGenCmras Σ} n :=
  match n with
    | S n =>
  (∃ I : gmap positive ((list (iProp Σ) * bi_schema) * list (iProp Σ)),
        (∃ γs, fmlist_idx inv_list_name n γs ∗
...
Global Arguments bi_schema_interp {_ _ _} _ _ _ _.
Global Arguments wsat {_ _ _} _.
  
Lemma wsat_unfold `{!invGS Σ} {Ω : gGenCmras Σ} n:
  wsat n =
  match n with
    | S n =>
  (∃ I : gmap positive ((list (iProp Σ) * bi_schema) * list (iProp Σ)),
...
Lemma bi_schema_interp_unfold `{!invGS Σ} {Ω : gGenCmras Σ} n Ps Ps_mut sch :
  ...
  | bi_sch_plainly sch => ■ bi_schema_interp n Ps Ps_mut sch
...
Proof.
  ...
  rewrite ?HeqQ ?HeqP //=.
  { rewrite //= ?later_equivI //=. iApply later_plainly_1. iNext. by iRewrite "Hequiv". }
Qed.
...
Proof.
  ...
  iDestruct (big_sepM_delete _ _ i with "HI") as "[[[#HPc [HQ [HQ_mut $]]]|HiE'] HI]"; eauto.
  - iSplitR "HQ HQ_mut"; last first.
    { iExists (length Qs_mut), (list_to_vec Qs_mut). iFrame.
      rewrite //=. rewrite vec_to_list_to_vec.
      iSplitR.
...
Proof.
  rewrite /ownI wsat_unfold.
  iIntros "((Hw&$) & Hi & Hi_mut & #HPc & HP & HiD)". iDestruct "Hw" as (I) "[Hw HI]".
  iDestruct "Hw" as (γs1) "(#Hidx1&Hw)".
  iDestruct "Hi" as (γs2) "(#Hidx2&Hi)".
...
Proof.
  ...
  rewrite ?HeqQ ?HeqP //=.
  rewrite //= ?later_equivI //=. iApply later_plainly. iNext. by iRewrite "Hagree".
Qed.
...
Proof.
  ...
  rewrite agree_op_invI /inv_mut_unfold.
  iRewrite -"Hval2b" in "Hown".
  rewrite agree_idemp.
  eauto.
Qed.
...
Proof.
  ...
  iDestruct (big_sepM_insert_acc _ _ i with "HI") as "[[[#Hcond [HQ [HQmut ?]]]|$] HI]"; eauto.
  { iDestruct (ownD_singleton_twice with "[$]") as %[]. }
  ...
  { simpl. rewrite HIlookup in HIlookup'. inversion HIlookup'; subst.
    iApply (bi_schema_interp_ctx_later with "HPQ []"); last eauto.
    {iClear "Hidx1 Hidx2 Hidx3". iModIntro; iIntros; iNext. trivial. }
  }
...
Proof.
  ...
  rewrite /ownD. iApply (big_sepM_insert _ I); first done.
  iFrame "HI Hcond". iLeft. rewrite /ownD. iFrame "∗ #".
...
Proof.
  ...
  iDestruct "Hw" as "(Hw&$)". rewrite /ownD.
  iDestruct "Hw" as (I) "[Hw HI]".
  iDestruct "Hw" as (γs) "(#Hidx1&Hw)".
...
Lemma ownD_nextgen `{!ngInvG Σ Ω} E :
  ownD E ⊢ ⚡==> ownD E.
Proof.
  rewrite /ownD.
  iIntros "Hown".
...
Lemma ownE_nextgen `{!ngInvG Σ Ω} E :
  ownE E ⊢ ⚡==> ownE E.
Proof.
  rewrite /ownE.
  iIntros "Hown".
...
#[global]
  Instance into_nextgen_ownD `{!ngInvG Σ Ω} E : IntoNextgen _ _ :=
  ownD_nextgen E.
#[global]
...
Lemma wsat_alloc `{!invGpreS Σ} {Ω : gGenCmras Σ} `{LC: !lcGS Σ} :
  ⊢ |==> ∃ (_ : invGS Σ) (_ : LC = invGS_lc) (_ : inv_inG = invGpreS0), wsat_all ∗ ownE ⊤.
Proof.
...
Definition bi_sch_bupd_factory (Q P: bi_schema) : bi_schema :=
  bi_sch_sep Q (bi_sch_plainly (bi_sch_wand Q (bi_sch_bupd (bi_sch_sep Q P)))).
...
Proof.
  ...
  iDestruct "H" as (??) "(#Hcond&Hinterp&HI_mut2)".
  ...
  { iClear "HI". iModIntro. iIntros. iNext. eauto. }
  repeat (rewrite ?bi_schema_interp_unfold //=).
  iSplitL "Hinterp".
  { rewrite ?list_lookup_fmap.
    ...
    * move: Hp. rewrite Heq => //= <-.
      { iDestruct "Hinterp" as "(?&#Hwand)". iSplitL ""; first eauto.
        iClear "HI". iModIntro. iIntros. iMod ("Hwand" with "[//]") as "(?&$)"; eauto. }
   }
  iExists _, _. iFrame "#".
