...
Definition uPred_fupd_split_level_def `{!invGS Σ} `{Ω : gGenCmras Σ} (E1 E2 : coPset) (k : nat) mj (P : iProp Σ) : iProp Σ :=
  wsat (S k) ∗ ownE (AE (S k) mj ∪ MaybeEn1 E1) ==∗ ◇ (wsat (S k) ∗ ownE (AE (S k) mj ∪ MaybeEn1 E2) ∗ P).
Definition uPred_fupd_split_level_aux `{!invGS Σ} `{Ω : gGenCmras Σ} : seal uPred_fupd_split_level_def. Proof. by eexists. Qed.
Definition uPred_fupd_split_level `{!invGS Σ} `{Ω : gGenCmras Σ} := uPred_fupd_split_level_aux.(unseal).
Definition uPred_fupd_split_level_eq `{!invGS Σ} `{Ω : gGenCmras Σ} : uPred_fupd_split_level = uPred_fupd_split_level_def :=
  uPred_fupd_split_level_aux.(seal_eq).

Definition uPred_fupd_level_def `{!invGS Σ} `{Ω : gGenCmras Σ} (E1 E2 : coPset) (k : nat) (P : iProp Σ) : iProp Σ :=
  uPred_fupd_split_level E1 E2 k None P.
Definition uPred_fupd_level_aux `{!invGS Σ} `{Ω : gGenCmras Σ} : seal uPred_fupd_level_def. Proof. by eexists. Qed.
...
Proof using Ω. iIntros (?). iApply ownE_mono_le_acc. by apply AE_subset_mono. Qed.

Lemma ownE_AlwaysEn_le_acc n1 mj1 :
  ownE (AlwaysEn) -∗ ownE (AE n1 mj1) ∗ (ownE (AE n1 mj1) -∗ ownE AlwaysEn).
Proof using Ω. iApply ownE_mono_le_acc. by apply AE_subset_AlwaysEn. Qed.
