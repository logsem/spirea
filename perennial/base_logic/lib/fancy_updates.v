...
Definition uPred_fupd_def `{!invGS Σ} `{Ω : gGenCmras Σ} (E1 E2 : coPset) (P : iProp Σ) : iProp Σ :=
...
Global Arguments uPred_fupd {Σ _ Ω}.
Lemma uPred_fupd_eq `{!invGS Σ} `{Ω : gGenCmras Σ} : @fupd _ uPred_fupd = uPred_fupd_def.
...
Lemma ownE_op_MaybeEn `{!invGS Σ} `{Ω : gGenCmras Σ} E1 E2 :
...
Lemma ownE_op_MaybeEn2 `{!invGS Σ} `{Ω : gGenCmras Σ} E1 E2 :
...
Lemma uPred_fupd_mixin `{!invGS Σ} `{Ω : gGenCmras Σ} : BiFUpdMixin (uPredI (iResUR Σ)) uPred_fupd.
Proof.
  split.
...
Qed.
Global Instance uPred_bi_fupd `{!invGS Σ} `{Ω : gGenCmras Σ} : BiFUpd (uPredI (iResUR Σ)) :=
  {| bi_fupd_mixin := uPred_fupd_mixin |}.

Global Instance uPred_bi_bupd_fupd `{!invGS Σ} `{Ω : gGenCmras Σ} : BiBUpdFUpd (uPredI (iResUR Σ)).
Proof. rewrite /BiBUpdFUpd uPred_fupd_eq. by iIntros (E P) ">? [$ $] !> !>". Qed.

Lemma ownE_mono_le_acc `{!invGS Σ} `{Ω : gGenCmras Σ} E1 E2:
  E1 ⊆ E2 →
...
Lemma ownE_weaken `{!invGS Σ} `{Ω : gGenCmras Σ} E1 E2 : E2 ⊆ E1 → ownE E1 -∗ ownE E2.
Proof.
...
Lemma lc_fupd_elim_later `{!invGS Σ} `{Ω : gGenCmras Σ} E P :
   £1 -∗ (▷ P) -∗ |={E}=> P.
Proof.
  iIntros "Hf Hupd".
  rewrite uPred_fupd_eq /uPred_fupd_def.
  iIntros "[$ $]". iApply (le_upd.le_upd_later with "Hf").
  iNext. by iModIntro.
Qed.
...
Lemma lc_fupd_add_later `{!invGS Σ} `{Ω : gGenCmras Σ} E1 E2 P :
  £1 -∗ (▷ |={E1, E2}=> P) -∗ |={E1, E2}=> P.
Proof.
...
Proof.
  ...
  iIntros "[Hone Hn]". rewrite -le_upd.le_upd_trans.
  iApply le_upd.bupd_le_upd.
  iMod wsat_alloc as (Hinv Hlc Hinvpre) "[Hw HE]".
  rewrite Hlc.
