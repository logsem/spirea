...
key lemma. *)
Definition own_discrete_fupd_def `{!invGS Σ} `{Ω : gGenCmras Σ} (P: iProp Σ) := own_discrete (|0={∅}=> P).
Definition own_discrete_fupd_aux `{!invGS Σ} `{Ω : gGenCmras Σ} : seal own_discrete_fupd_def. Proof. by eexists. Qed.
Definition own_discrete_fupd `{!invGS Σ} `{Ω : gGenCmras Σ} := own_discrete_fupd_aux.(unseal).
Definition own_discrete_fupd_eq `{!invGS Σ} `{Ω : gGenCmras Σ} : own_discrete_fupd = own_discrete_fupd_def :=
  own_discrete_fupd_aux.(seal_eq).
Arguments own_discrete_fupd {_ _ _} _%I.

Class IntoDiscreteFupd `{!invGS Σ} `{Ω : gGenCmras Σ} (P Q : iProp Σ) :=
  into_discrete_fupd : P ⊢ own_discrete_fupd Q.
Arguments IntoDiscreteFupd {_ _ _} _%I _%I.
Arguments into_discrete_fupd {_ _ _} _%I _%I {_}.
#[global]
...
Section own_disc_fupd_props.
  ...
  Lemma modality_own_discrete_fupd_mixin :
    modality_mixin (@own_discrete_fupd Σ _ _)
                   (MIEnvId)
