...
Definition wpc_pre `{!irisGS Λ Σ Ω, !generationGS Λ Σ} (s : stuckness) (mj: fracR)
...
Local Instance wpc_pre_contractive `{!irisGS Λ Σ Ω, !generationGS Λ Σ} s mj : Contractive (wpc_pre s mj).
Proof.
...
Definition wpc0 `{!irisGS Λ Σ Ω, !generationGS Λ Σ} (s : stuckness) mj :
  coPset → expr Λ → (val Λ → iProp Σ) → iProp Σ → iProp Σ := fixpoint (wpc_pre s mj).

Definition wpc_def `{!irisGS Λ Σ Ω, !generationGS Λ Σ} (s : stuckness) :
  coPset → expr Λ → (val Λ → iProp Σ) → iProp Σ → iProp Σ :=
  λ E1 e1 Φ Φc, (∀ mj, wpc0 s mj E1 e1 Φ Φc)%I.
Definition wpc_aux `{!irisGS Λ Σ Ω, !generationGS Λ Σ} : seal (@wpc_def Λ Σ Ω _ _). by eexists. Qed.

Definition wpc `{!irisGS Λ Σ Ω, !generationGS Λ Σ} := wpc_aux.(unseal).
Arguments wpc {Λ Σ Ω _ _} _ E _%E _%I _%I.
#[global]
Instance: Params (@wpc) 8 := {}.
Definition wpc_eq `{!irisGS Λ Σ Ω, !generationGS Λ Σ} : wpc = @wpc_def Λ Σ Ω _ _ := wpc_aux.(seal_eq).
...
*)
Definition wp_def `{!irisGS Λ Σ Ω, !generationGS Λ Σ} : Wp (iProp Σ) (expr Λ) (val Λ) stuckness :=
  λ s E e Φ, (WPC e @ s ; E {{ Φ }} {{ True }})%I.
Definition wp_aux : seal (@wp_def). Proof. by eexists. Qed.
Definition wp' := wp_aux.(unseal).
Global Arguments wp' {Λ Σ Ω _}.
...
Lemma wp_eq `{!irisGS Λ Σ Ω, !generationGS Λ Σ} : wp = @wp_def Λ Σ Ω _ _.
...
Section wpc.
Context `{!irisGS Λ Σ Ω, !generationGS Λ Σ}.
...
Context {Λ : ectxLanguage} `{!irisGS Λ Σ Ω, !generationGS Λ Σ} {Hinh : Inhabited (state Λ)}.
