...
Definition ae_inv_mut_def `{!invGS Σ} `{Ω : gGenCmras Σ} (k: nat) mj sch (Ps : list (iProp Σ)) : iProp Σ :=
    ∃ i, ⌜i ∈ AE_next_diff (S k) mj⌝ ∧ ownI k i sch (list_to_vec Ps).
Definition ae_inv_mut_aux : seal (@ae_inv_mut_def). Proof. by eexists. Qed.
Definition ae_inv_mut {Σ Ω i} := ae_inv_mut_aux.(unseal) Σ Ω i.
Definition ae_inv_mut_eq : @ae_inv_mut = @ae_inv_mut_def := ae_inv_mut_aux.(seal_eq).
#[global]
Instance: Params (@ae_inv_mut) 4 := {}.
Typeclasses Opaque ae_inv_mut.

Definition ae_inv_mut_full_def `{!invGS Σ} `{Ω : gGenCmras Σ} (k: nat) mj sch (Qs Ps : list (iProp Σ)) : iProp Σ :=
...
Definition ae_inv_mut_full {Σ Ω i} := ae_inv_mut_full_aux.(unseal) Σ Ω i.
Definition ae_inv_mut_full_eq : @ae_inv_mut_full = @ae_inv_mut_full_def := ae_inv_mut_full_aux.(seal_eq).
#[global]
Instance: Params (@ae_inv_mut_full) 4 := {}.
...
Section ae_inv_mut.
  ...
  Proof.
    ...
    iDestruct "HI" as (? Qs_mut) "(#Hcond&Hinterp&Hmut)".
    ...
    { iClear "HiP". iModIntro. iIntros. iNext. eauto. }
    ...
    { destruct k.
      - iMod ("Hshift" with "[$]") as ">(HP&HR)".
        ...
        { rewrite ?vec_to_list_to_vec. iFrame "HP Hcond'". }
