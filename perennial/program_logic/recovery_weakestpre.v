...
Definition wpr_pre `{!irisGS Λ Σ Ω} (CS: crash_semantics Λ) (s : stuckness)
...
Local Instance wpr_pre_contractive `{!irisGS Λ Σ Ω} CS s: Contractive (wpr_pre CS s).
Proof.
...
Definition wpr_def `{!irisGS Λ Σ Ω} CS (s : stuckness) :
...
Definition wpr_aux `{!irisGS Λ Σ Ω} : seal (@wpr_def Λ Σ Ω _). by eexists. Qed.
Definition wpr `{!irisGS Λ Σ Ω} := wpr_aux.(unseal).
Definition wpr_eq `{!irisGS Λ Σ Ω} : wpr = @wpr_def Λ Σ Ω _ := wpr_aux.(seal_eq).
...
Local Instance wpr0_pre_contractive `{!irisGS Λ Σ Ω} CS s mj: Contractive (wpr0_pre CS s mj).
Proof.
...
Definition wpr0_def `{!irisGS Λ Σ Ω} CS (s : stuckness) (mj : fracR) :
  generationGS Λ Σ → coPset → expr Λ → expr Λ →
  (val Λ → iProp Σ) →
  (generationGS Λ Σ → iProp Σ) →
  (generationGS Λ Σ → val Λ → iProp Σ) → iProp Σ
  := fixpoint (wpr0_pre CS s mj).
Definition wpr0_aux `{!irisGS Λ Σ Ω} : seal (@wpr0_def Λ Σ Ω _). by eexists. Qed.
Definition wpr0 `{!irisGS Λ Σ Ω} := wpr0_aux.(unseal).
Definition wpr0_eq `{!irisGS Λ Σ Ω} : wpr0 = @wpr0_def Λ Σ Ω _ := wpr0_aux.(seal_eq).
