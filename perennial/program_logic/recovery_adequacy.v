...
Lemma step_fupdN_fresh_rearrange {Λ Σ} `{!irisGS Λ Σ Ω} {HG : generationGS Λ Σ} φ ns ncurr k k2:
...
Lemma step_fupdN_fresh_soundness {Λ Σ Ω} `{!invGpreS Σ} `{!lcGpreS Σ} `{!crashGpreS Σ} (φ : Prop) ns ncurr k k2 f g:
  (∀ (Hi: invGS Σ) (Hc: crashGS Σ), NC 1 ={⊤}=∗
    ∃ (HI: irisGS Λ Σ Ω) (HG:generationGS Λ Σ) (Hpf1: iris_invGS = Hi)
...
Corollary wp_recv_adequacy_inv Σ Ω Λ CS `{!invGpreS Σ} `{!lcGpreS Σ} `{!crashGpreS Σ} nsinit s e r σ g φ φr φinv f1 f2:
  (∀ `(Hinv : !invGS Σ) `(Hc: !crashGS Σ) κs,
     ⊢ |={⊤}=> ∃
         ...
         Φinv,
        let HI := IrisGS Λ Σ Ω Hinv (global_stateI) (fork_post) f1 f2 Hpf1a Hpf1b in
...
Proof.
  ...
  iModIntro.
  set (HI := IrisGS Λ Σ Ω Hinv (global_stateI) (fork_post) f1 f2 Hpf1a Hpf1b).
