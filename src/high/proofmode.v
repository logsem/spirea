...
Lemma tac_wp_expr_eval `{!nvmG Σ, nvmDeltaG} Δ s E Φ e e' : (∀ (e'':=e'), e = e'') →
...
Lemma tac_wp_pure `{!nvmG Σ, nvmDeltaG} Δ Δ' s E K e1 e2 φ n Φ :
...
Qed.
Lemma tac_wp_pure_no_later `{!nvmG Σ, nvmDeltaG} Δ s E K e1 e2 φ n Φ :
  (∀ TV, PureExec φ n (ThreadState e1 TV) (ThreadState e2 TV)) →
  φ →
  envs_entails Δ (WP (fill K e2) @ s; E {{ Φ }}) →
  envs_entails Δ (WP (fill K e1) @ s; E {{ Φ }}).
...
Lemma tac_wp_value_nofupd `{!nvmG Σ, nvmDeltaG} Δ s E Φ v :
...
Lemma tac_wp_value `{!nvmG Σ, nvmDeltaG} Δ s E (Φ : val → dPropI Σ) v :
...
Lemma tac_wp_bind `{!nvmG Σ, nvmDeltaG} K Δ s E Φ e f :
