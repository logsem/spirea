...
Definition wpr_pre `{!nvmBaseG Σ Ω, !nvmHighG Σ Ω, !PerennialG Σ} (s : stuckness)
  ...
  (WPC e @ s ; E
    ...
    {{ ∀ σ mj D σ' (HC : crash_prim_step nvm_crash_lang σ σ') ns n, (* The [n] here actually doesn't matter. *)
      ⎡ state_interp σ n -∗ global_state_interp (Λ := nvm_lang) () ns mj D [] ={E}=∗
...
Lemma or_lost_post_crash_full `{nvmHighG} CV :
  crashed_at CV -∗
...
(*   interpretations _after_ a crash. *)
  ...
  Lemma nvm_reinit `{!nvmBaseG Σ Ω, !nvmHighG Σ Ω, !PerennialG Σ} n σ σ':
    ...
    nvm_heap_ctx σ'.
  Proof.
(*     iIntros ([store PV CV pIncl cut]) "[nvm_heap extra]". *)
(*     iMod (heap_ctx_next_generation with "nvm_heap") as "nvm_heap"; first done. *)
(*     do 3 iModIntro. *)
...
(*       done. } *)
  ...
  Lemma idempotence_wpr `{!nvmBaseG Σ Ω, !nvmHighG Σ Ω, !PerennialG Σ} s E1 e rec Φ Φr (Φc : dProp Σ) `{!Objective Φc} :
    ⊢ WPC e @ s ; E1 {{ Φ }} {{ Φc }} -∗
      (<obj> ■ (Φc -∗ <NG> (WPC rec @ s ; E1 {{ Φr }} {{ Φc }}))) -∗
      wpr s E1 e rec Φ Φr.
  Proof.
    iModel.
    iRevert (e Φ TV).
    ...
    iIntros .
    rewrite /wpr wpr_unfold /wpr_pre.
    iApply (wpc_strong_mono' with "wpc"); [ by auto | by auto | ].
    iSplit. { iIntros (?). monPred_simpl. setoid_rewrite monPred_at_fupd. auto. }
    monPred_simpl.
    iIntros (TV'' ?) "Φc".
    iModIntro.
    iIntros (???? step ns ?) "state global".
    iMod (nvm_reinit with "state") as "state"; first done.
    iModIntro.
    iDestruct ("Hidemp" with "Φc") as "Hidemp'".
    iEval (rewrite /high.modalities.nextgen.nextgen /=) in "Hidemp'".
    iModIntro.
    iMod "state" as "(#validV & interp & nvm_heap_ctx)".
    iModIntro.
