...
Section big_sepM2.
  ...
  (* The following lemma is generated by Claude code. *)
  Lemma big_sepM2_union `{Countable K}
    (Φ : K → A → B → PROP) (m1 m2 : gmap K A) (n1 n2 : gmap K B) :
    m1 ##ₘ m2 →
    ([∗ map] k↦x;y ∈ m1;n1, Φ k x y) -∗
    ([∗ map] k↦x;y ∈ m2;n2, Φ k x y) -∗
    ([∗ map] k↦x;y ∈ (m1 ∪ m2);(n1 ∪ n2), Φ k x y).
  Proof.
    generalize dependent n1.
    induction m1 as [|k x m1 Hk IH] using map_ind; iIntros (n1 Hdisj) "H1 H2".
    - iDestruct (big_sepM2_dom with "H1") as %Hdom.
      rewrite dom_empty_L in Hdom. symmetry in Hdom. apply dom_empty_inv_L in Hdom as ->.
      rewrite !left_id_L. iFrame "H2".
    - apply map_disjoint_insert_l in Hdisj as [Hm2k Hdisj].
      iDestruct (big_sepM2_dom with "H2") as %Hdom2.
      rewrite big_sepM2_delete_l; last apply lookup_insert_eq.
      iDestruct "H1" as (y Hy) "[HΦ H1]".
      assert (delete k (<[k:=x]> m1) = m1) as Heq by (apply delete_insert_id; done).
      iEval (rewrite Heq) in "H1".
      iDestruct (IH (delete k n1) Hdisj with "H1 H2") as "IH".
      assert ((m1 ∪ m2) !! k = None) as Hunionk.
      { apply lookup_union_None. split; [done|done]. }
      assert ((delete k n1 ∪ n2) !! k = None) as Hnunionk.
      { apply lookup_union_None. split; [apply lookup_delete_eq|].
        apply not_elem_of_dom. rewrite -Hdom2. by apply not_elem_of_dom. }
      rewrite -insert_union_l.
      assert (n1 ∪ n2 = <[k:=y]> (delete k n1 ∪ n2)) as Hn.
      { rewrite insert_union_l insert_delete_eq. by rewrite insert_id. }
      rewrite Hn big_sepM2_insert //.
      iFrame "HΦ IH".
  Qed.
End big_sepM2.
