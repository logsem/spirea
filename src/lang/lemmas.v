...
Lemma prim_step_flush_no_fork ℓ SV PV BV (σ1 : state nvm_lang)
  (g1 : global_state nvm_lang) (κ : list (language.observation nvm_lang))
  (e2 : language.expr nvm_lang) (σ2 : state nvm_lang) (g2 : global_state nvm_lang)
  (efs : list (language.expr nvm_lang)) :
  prim_step (Flush #ℓ `at` (SV, PV, BV)) σ1 g1 κ e2 σ2 g2 efs → efs = [].
Proof.
    intros [Ki [??] [??] ? ? step].
    subst.
    simpl in *.
...
Lemma prim_step_cmpxchg_no_fork ℓ (v_i v_t : val) SV PV BV
  ...
  (e2 : language.expr nvm_lang) (σ2 : state nvm_lang) (g2 : global_state nvm_lang)
  (efs : list (language.expr nvm_lang)) :
  prim_step (CmpXchg #ℓ v_i v_t `at` (SV, PV, BV)) σ1 g1 κ e2 σ2 g2 efs → efs = [].
Proof.
  intros [Ki [??] [??] ? ? step].
  subst.
  simpl in *.
...
Lemma prim_step_faa_no_fork ℓ (v : val) SV PV BV
  (σ1 : state nvm_lang) (g1 : global_state nvm_lang) (κ : list (language.observation nvm_lang))
  (e2 : language.expr nvm_lang) (σ2 : state nvm_lang) (g2 : global_state nvm_lang)
  (efs : list (language.expr nvm_lang)) :
  prim_step (FAA #ℓ v `at` (SV, PV, BV)) σ1 g1 κ e2 σ2 g2 efs → efs = [].
Proof.
  intros [Ki [??] [??] ? ? step].
  subst.
  simpl in *.
