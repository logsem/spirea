...
Lemma wptp_steps s (n : nat) e1 t1 κs κs' t2 σ1 g1 ns D σ2 g2 Φ Φc :
...
Proof.
  ...
  iIntros (Hsteps) "Hσ Hg He Ht Hlc". inversion_clear Hsteps as [|?? [t1' [σ1' g1']]].
  rewrite -(assoc_L (++)).
  rewrite -Nat.add_succ_l lc_split.
  iDestruct "Hlc" as "[Hlc1 Hlc2]".
  iApply step_fupd2N_inner_add.
  iMod (wptp_step with "Hσ Hg He Ht Hlc1") as (e1' t1'' ?) ">H"; first eauto; simplify_eq.
  iApply (step_fupd2N_wand with "H"). iIntros "!> H".
  iMod "H".
  iDestruct "H" as "(Hσ & Hg & He & Ht)".
  iMod (IH with "Hσ Hg He Ht Hlc2") as "IH"; first done.
  rewrite -Nat.iter_succ_r.
  iModIntro. iModIntro. eauto.
Qed.

Lemma wpc_safe κs m e σ g ns D Φ Φc :
  state_interp σ m -∗
...
Proof.
  iIntros (Hstep) "Hσ Hg He Ht Hlc".
  iMod (wptp_steps with "Hσ Hg He Ht Hlc") as "Hwp"; first done.
  ...
  iMod (fupd2_mask_subseteq ⊤ (⊤ ∖ D)) as "Hclo"; try set_solver+.
  iMod (wpc0_value_inv_option with "Hwp Hg") as "[$ Hg]".
  ...
  + apply of_to_val in He as <-.
    iMod (fupd2_mask_subseteq ⊤ (⊤ ∖ D)) as "Hclo'"; try set_solver+.
    iMod (wpc0_value_inv' with "Hv Hg") as "($ & ?)".
    iMod "Hclo'" as "_".
    by iApply ("IH" with "[$] [$]").
  + by iApply ("IH" with "[$] [$]").
Qed.
...
Local Lemma wptp0_progress Φ Φc κs' n e1 t1 κs t2 σ1 g1 ns D σ2 g2 e2 :
  ...
  ||▷=>^(num_laters_per_step (Nat.iter n step_count_next ns) + 1)
   ⌜ not_stuck e2 σ2 g2 ⌝.
Proof.
  iIntros (Hstep Hel) "Hσ Hg He Ht Hlc".
  ...
  - iDestruct "Ht" as "(_ & He' & _)".
    iPoseProof (wpc_safe with "Hσ Hg He' Hlc2") as "H".
    by iMod "H".
Qed.
...
Local Lemma wptp0_strong_crash_adequacy Φ Φc κs' s n e1 t1 κs t2 σ1 g1 ns D σ2 g2 :
  ...
  ▷ (∃ e2 t2',
    ⌜ t2 = e2 :: t2' ⌝ ∗
    Φc ∗ state_interp σ2 (length t2') ∗ global_state_interp g2 ((Nat.iter n step_count_next ns)) mj D κs').
Proof.
  iIntros (Hstep) "Hσ Hg He Ht Hlc".
  rewrite steps_sum_S_r.
  iDestruct "Hlc" as "[Hlc1 Hlc2]".
  iDestruct (wptp_steps with "Hσ Hg He Ht Hlc1") as "Hwp"; first done.
  iApply step_fupd2N_inner_add.
  iApply (step_fupd2N_inner_wand with "Hwp"); auto.
  iDestruct 1 as (e2' t2' ?) "(Hσ & Hg & Hwp & Ht)"; simplify_eq/=.
  iMod (fupd2_mask_subseteq ⊤ (⊤ ∖ D)) as "Hclo"; try set_solver+.
